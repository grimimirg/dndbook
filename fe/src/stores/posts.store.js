...
export const usePostsStore = defineStore('posts', () => {
    ...
    async function uploadImage(postId, file, description = null, orderIndex = null) {
        try {
    ...
    async function updateImageDescription(postId, imageId, description, orderIndex = null) {
        try {
            const data = {description};
            if (orderIndex !== null) {
                data.order_index = orderIndex;
            }
            const response = await api.post(`/posts/${postId}/images/${imageId}`, data);
            return {success: true, image: response.data};
        } catch (error) {
            return {
                success: false,
    ...
    async function reorderImages(postId, imageOrders) {
        try {
            await api.put(`/posts/${postId}/images/reorder`, {image_orders: imageOrders});
            return {success: true};
        } catch (error) {
            return {
                success: false,
                error: error.response?.data?.error || 'Failed to reorder images'
            };
        }
    }
